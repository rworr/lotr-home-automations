...
func TestNewGearList(t *testing.T) {
	...
	golburzGear := gearlist.NewGearList(characterGear)

	gearInfo := gearlist.GearInfo{Name: "Armor Essence II", Location: " 3-7"}
	characterLevel := gearlist.CharacterGearLevel{Character: "Golburz", Level: "Gear Level 4"}
	if golburzGear[gearInfo][characterLevel] != 4 {
		t.Errorf("Expected quantity 4, found %v\n", golburzGear[gearInfo][characterLevel])
	}
}
